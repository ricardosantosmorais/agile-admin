...
- `Catálogos Digitais` possui superfície v2 em `/catalogos-digitais`, com mapeamento do componente dinâmico legado `catalogos-studio`, bridge server-side para `catalogos_digitais`, alerta de contratação da Agile Store (`mod_catalogos_digitais`), listagem alinhada ao legado com código, nome, vigência e status, filtros recolhidos no padrão v2 e bloqueio do botão `Novo catálogo` quando o módulo não está contratado. A edição já usa o fluxo passo a passo Geral, Blocos e Resumo; edição completa de blocos/produtos, preview, PDF e publicação permanece como próxima fatia do mesmo módulo.
