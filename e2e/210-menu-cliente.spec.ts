...
test.describe('Menu do cliente', () => {
  test('abre meus atendimentos, base de conhecimento e atualizacoes gerais no v2', async ({ page }) => {
    await page.route(/\/api\/meus-atendimentos(?:\?.*)?$/, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          data: [{ id: 'conv-1', protocolo: 'conv-1', data_abertura: 1712750400, data_encerramento: 0, status: 'open' }],
          meta: { page: 1, pages: 1, perPage: 5, from: 1, to: 1, total: 1 },
          nextCursor: null,
        }),
      })
    })
    ...
    await page.route(/\/api\/meus-atendimentos\/conv-1$/, async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          dataEncerramento: 0,
          timeline: [{ id: 'part-1', authorName: 'Cliente Teste', authorType: 'contact', body: '<p>Mensagem inicial</p>', createdAt: 1712750400, partType: 'comment' }],
        }),
      })
    })
    ...
    await page.route('/api/base-conhecimento**', async (route) => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          data: [{ id: 'art-1', titulo: 'Como alterar a senha', descricao: 'Passo a passo', dataCriacao: 1712750400, html: '<p>Conteudo do artigo</p>' }],
    ...
    await page.route('/api/atualizacoes-gerais**', async (route) => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          data: [{ id: 'chg-1', titulo: 'Melhoria no catalogo', data: '2026-04-10 00:00:00', plataforma: 'admin', tipo: 'melhoria', apenasMaster: false, conteudo: '<p>Novo resumo visual.</p>' }],
          meta: { total: 1 },
        }),
      })
    })
    ...
    await page.locator('tbody tr').filter({ hasText: 'conv-1' }).first().locator('button').first().click()
    ...
    await expect(page.getByText(/changelog do produto|product changelog/i)).toBeVisible()
    await expect(page.getByText(/Melhoria no cat.logo|Melhoria no catalogo/i)).toBeVisible()
  })
})
