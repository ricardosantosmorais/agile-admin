...
test.describe('Integrações > Notificações', () => {
	test('carrega parâmetros do Firebase e salva alteração', async ({ page }) => {
		...
		await page.goto('/integracoes/notificacoes', {
		...
		await page
			.getByLabel(/código do projeto|project code/i)
			.first()
			.fill('firebase-app-updated');
		await page
