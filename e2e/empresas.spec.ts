...
import { fieldDateInput, fieldInput, fieldNumberInput, fieldSelect } from '@/e2e/helpers/crud'
...
test.describe('Empresas', () => {
  test('lista empresas, cria um novo registro e abre a edição', async ({ page }) => {
    ...
    await fieldInput(page, /^cep$/i).fill('29160-000')
    await fieldInput(page, /^endere[cç]o$/i).fill('Rua Central')
    await fieldInput(page, /^n[uú]mero$/i).fill('100')
    await fieldInput(page, /^bairro$/i).fill('Centro')
    await fieldInput(page, /^cidade$/i).fill('Serra')
    await fieldSelect(page, /^uf$/i).selectOption('ES')
    await fieldInput(page, /^e-mail$/i).first().fill('contato@empresa55.com.br')
    await fieldInput(page, /^telefone$/i).fill('(27) 3333-4444')
    await fieldInput(page, /^celular$/i).fill('(27) 99999-9999')
    await fieldInput(page, /^url$/i).fill('empresa55.exemplo.com.br')
    await fieldSelect(page, /^cluster$/i).selectOption('13')
    await fieldInput(page, /^bucket s3$/i).fill('bucket-root')
    await fieldSelect(page, /^erp$/i).selectOption('sap')
    await fieldSelect(page, /^template de integra[cç][aã]o$/i).selectOption('9')
    await fieldSelect(page, /^gerente de implanta[cç][aã]o$/i).selectOption('1')
    await fieldSelect(page, /^analista de implanta[cç][aã]o$/i).selectOption('2')
    ...
    await expect(fieldInput(page, /^e-mail$/i).first()).toHaveValue('contato@empresa55.com.br')
  })
})
