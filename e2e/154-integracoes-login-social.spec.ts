...
test.describe('Integracoes > Login Social', () => {
  test('carrega configuracoes e salva alteracao do Google', async ({ page }) => {
    ...
    await page.goto('/integracoes/login-social', { waitUntil: 'domcontentloaded', timeout: 60_000 })
    ...
    await expect(page.getByText('https://loja.test/components/social-login.php').first()).toBeVisible()

    await page.locator('main input').nth(1).fill('google-id-updated')
