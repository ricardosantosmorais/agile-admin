...
test('opens maintenance logs and displays details when records are available', async ({ page }) => {
  ...
  await expect(page.getByLabel(/módulo|module/i)).toBeVisible()
  await expect(page.getByLabel(/módulo|module/i)).toHaveJSProperty('tagName', 'SELECT')
  await expect(page.getByLabel(/^usuário$|^user$/i)).toHaveJSProperty('tagName', 'SELECT')
