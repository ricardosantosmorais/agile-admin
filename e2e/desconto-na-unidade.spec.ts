...
test('creates a unit discount campaign, loads tabs and deletes it', async ({ page }) => {
  const uniqueCode = `DU-${Date.now()}`
  const uniqueName = `Desconto ${uniqueCode}`

  await openList(page)
  await page.getByRole('link', { name: /novo|new/i }).click()
  await expect(page).toHaveURL(/\/desconto-na-unidade\/novo$/)

  await formTextInput(page, 0).fill(uniqueCode)
  await formTextInput(page, 1).fill(uniqueName)
  await formNumberInput(page, 0).fill('3')
  await formTextInput(page, 2).fill('15,50')
  await formNumberInput(page, 1).fill('10')
