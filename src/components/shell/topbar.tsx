  ...
  return (
    <>
      <header ref={rootRef} className="relative z-[1] rounded-[1.45rem] border border-[#ebe6d8] bg-white px-3.5 py-3 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                ...
                <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-full overflow-hidden rounded-[1.5rem] border border-line bg-white p-2 shadow-2xl">
                  ...
                  <div className="max-h-[320px] overflow-y-auto">
                    ...
                    {!filteredTenants.length ? (
                      <div className="px-3 py-4 text-sm text-slate-500">{t('shell.tenantEmpty')}</div>
                    ) : null}
                  </div>
                </div>
              ) : null}
                      ...
                      filteredQuickAccessItems.map((item) => {
                        ...
                        const content = (
                          <>
                            ...
                            <span className="rounded-full bg-[#eff7f3] px-2.5 py-1 text-[11px] font-semibold text-accent">
                              {t('common.open')}
                            </span>
                          </>
                        )
          ...
          <div className="flex flex-wrap items-center gap-2 xl:flex-nowrap">
            ...
            <IconTooltipButton label={integrationEnabled ? t('shell.integrationOnline') : t('shell.integrationPaused')} onClick={() => setIntegrationEnabled((current) => !current)}>
              ...
              {activePanel === 'user' ? (
                <div className="absolute right-0 top-12 z-50 w-[360px] rounded-[1.5rem] border border-line bg-white p-4 shadow-2xl">
                  ...
                  <div className="mt-4 space-y-2 text-sm">
                    <button type="button" className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left font-medium text-slate-700 transition hover:bg-surface hover:text-slate-950">
      ...
      {activePanel === 'notifications' ? (
        <div
          className="fixed inset-0 z-[60] flex items-start justify-end bg-slate-950/30 p-4 backdrop-blur-sm"
            ...
            <div className="mt-5 space-y-3">
              <AsyncState isLoading={notificationsLoading} error={notificationsError}>
                {notifications.length ? notifications.map((notification) => (
                  ...
                  <div className="rounded-[1.5rem] border border-dashed border-line bg-surface px-4 py-6 text-center text-sm text-slate-500">
