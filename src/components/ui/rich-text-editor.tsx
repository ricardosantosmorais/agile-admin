...
export function RichTextEditor({ value, onChange, disabled = false }: RichTextEditorProps) {
	...
	const editor = useEditor({
		...
		editorProps: {
			attributes: {
				class:
					'min-h-[420px] max-h-[620px] w-full overflow-y-auto px-6 py-5 text-[15px] leading-8 text-[color:var(--app-text)] outline-none [&_h1]:mb-4 [&_h1]:mt-6 [&_h1]:text-[2rem] [&_h1]:font-bold [&_h2]:mb-4 [&_h2]:mt-6 [&_h2]:text-[1.75rem] [&_h2]:font-bold [&_h3]:mb-3 [&_h3]:mt-5 [&_h3]:text-[1.45rem] [&_h3]:font-semibold [&_h4]:mb-3 [&_h4]:mt-5 [&_h4]:text-[1.2rem] [&_h4]:font-semibold [&_h5]:mb-3 [&_h5]:mt-5 [&_h5]:text-[1.05rem] [&_h5]:font-semibold [&_h6]:mb-2 [&_h6]:mt-4 [&_h6]:text-[0.98rem] [&_h6]:font-semibold [&_p]:my-3 [&_div]:my-3 [&_ul]:my-4 [&_ul]:list-disc [&_ul]:pl-7 [&_ol]:my-4 [&_ol]:list-decimal [&_ol]:pl-7 [&_li]:my-1.5 [&_a]:font-medium [&_a]:text-emerald-600 [&_a]:underline [&_strong]:font-semibold [&_u]:underline [&_img]:my-4 [&_img]:max-w-full [&_img]:rounded-xl',
			},
		},
	})
	...
	return (
		<div className="app-pane w-full overflow-hidden rounded-[1rem]">
			...
			<div className="flex flex-wrap items-center gap-2 border-b border-[color:var(--app-card-border)] px-3 py-3">
				...
				<button type="button" className={toolbarButtonClass(editor.isActive('underline'), disabled)} onClick={() => editor.chain().focus().toggleUnderline().run()} disabled={disabled}><Underline className="h-4 w-4" /></button>
			...
			<EditorContent
				...
				className={
					disabled
						? 'app-control-muted text-[color:var(--app-muted)] [&_.ProseMirror]:bg-transparent'
						: '[&_.ProseMirror]:bg-transparent'
				}
				onBlur={() => void emitNormalizedHtml()}
			/>
		</div>
	)
}
