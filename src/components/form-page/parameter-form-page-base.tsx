...
export function ParameterFormPageBase<FormValues extends Record<string, string>, Collections extends LookupCollections, Context = undefined>({
  ...
  const effectiveFieldDefinitions = useMemo(
    () => resolveFieldDefinitions?.(context) ?? fieldDefinitions,
    [context, fieldDefinitions, resolveFieldDefinitions],
  )
                    ...
                    <div key={String(field.key)} className="rounded-[1.15rem] border border-[#ebe4d8] bg-[#fcfaf5] p-4">
                      <FormField label={field.label} asLabel={false}>
                        {field.type === 'lookup' ? (
                          <LookupSelect<LookupSelectOption>
                            ...
                            loadOptions={async (query, page, perPage) => {
                              ...
                              const filteredOptions = !normalizedQuery
                                ? lookupOptions
                                : lookupOptions.filter((option) => option.label.toLowerCase().includes(normalizedQuery))
                          ...
                          >
                            {field.includeEmptyOption !== false ? (
