...
function inferSessionLostReason(status: number, message: string): SessionLostReason {
	...
	if (normalized.includes('expirada') || normalized.includes('expirou') || normalized.includes('sessão não encontrada') || normalized.includes('sessao nao encontrada')) {
...
function shouldNotifySessionLoss(path: string) {
	...
	return !['/api/auth/logout', '/api/auth/session', '/api/auth/login'].some((prefix) => path.startsWith(prefix));
}

function canRequestWhileSessionBlocked(path: string) {
	return ['/api/auth/logout', '/api/auth/login', '/api/auth/session'].some((prefix) => path.startsWith(prefix));
}
...
export function notifySessionLost(reason: SessionLostReason, message: string, status = 401, path = '') {
	...
	window.dispatchEvent(
		new CustomEvent<SessionLostDetail>(SESSION_LOST_EVENT, {
			detail: {
				reason,
				message,
				status,
