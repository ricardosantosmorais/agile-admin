...
export function IntegracaoComErpTemplatesListPage() {
	const { session } = useAuth()
	const { t } = useI18n()

	if (!isRootAgileecommerceAdmin(session)) {
		return <AccessDeniedState title={t('maintenance.erpIntegration.catalogs.items.templates.title', 'Templates')} backHref="/dashboard" />
	}

	return <CrudListPage config={INTEGRACAO_COM_ERP_TEMPLATES_CONFIG} client={integracaoComErpTemplatesClient} />
}
