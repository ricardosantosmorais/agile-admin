...
describe('restricoes-produtos-mappers', () => {
  it('flattens audience and product criteria into crossed rows', () => {
    const rows = flattenWizardDraft({
      ...
      general: { ...restricaoProdutoDefaultDraft.general, perfil: 'cliente', ativo: true, orcamento: true, pedido_minimo: '100,00', motivo: 'Sem venda' },
    })
  ...
  it('computes updates and deletes from original rows', () => {
    const payload = buildWizardPayload({
      ...
      products: [{ id: 'prd-1', type: 'produto', values: [{ id: '10', label: 'Produto 10' }] }],
      general: { ...restricaoProdutoDefaultDraft.general, perfil: 'todos', ativo: true, orcamento: false, pedido_minimo: '', motivo: 'Bloqueio' },
    }, [
      { id: '500', perfil: 'todos', motivo: 'Bloqueio', id_produto: '10' },
      { id: '501', perfil: 'todos', motivo: 'Bloqueio', id_produto: '20' },
    ], '500')
