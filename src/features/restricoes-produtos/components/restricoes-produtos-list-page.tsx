...
type RestricoesProdutosFilters = CrudListFilters & {
  q: string
  perfil: string
  ativo: string
}

const DEFAULT_FILTERS: RestricoesProdutosFilters = {
  page: 1,
  perPage: 15,
  orderBy: 'created_at',
  sort: 'desc',
...
function pickLabel(record: CrudListRecord, field: string) {
  ...
  if (relation && typeof relation === 'object') {
    ...
    return String(entity.nome_fantasia || entity.nome || record[field] || '').trim()
  }
...
export function RestricoesProdutosListPage() {
  const { t } = useI18n()
  const access = useFeatureAccess('restricoesProdutos')
  const [filters, setFilters] = useState<RestricoesProdutosFilters>(DEFAULT_FILTERS)
  const [filtersDraft, setFiltersDraft] = useState<RestricoesProdutosFilters>(DEFAULT_FILTERS)
  const [confirmDeleteIds, setConfirmDeleteIds] = useState<string[] | null>(null)
  const [expandedRowIds, setExpandedRowIds] = useState<string[]>([])
  const [filtersExpanded, setFiltersExpanded] = useState(false)
  const [feedback, setFeedback] = useState<{ tone: 'success' | 'error'; message: string } | null>(null)

  const listState = useAsyncData(() => restricoesProdutosClient.list(filters), [filters])
  const rows = useMemo(() => listState.data?.data ?? [], [listState.data?.data])

  const tableState = useDataTableState<
    CrudListRecord,
    RestricoesProdutosFilters,
    RestricoesProdutosFilters['orderBy']
  >({
    rows,
  ...
  const columns = useMemo<AppDataTableColumn<CrudListRecord, RestricoesProdutosFilters>[]>(() => [
    {
      id: 'id',
    ...
    {
      id: 'perfil',
      label: t('maintenance.productRestrictions.fields.profile', 'Perfil'),
      sortKey: 'perfil',
      thClassName: 'w-[140px]',
      cell: (row) => <StatusBadge tone="neutral">{String(row.perfil || 'todos')}</StatusBadge>,
      filter: {
        kind: 'select',
        id: 'perfil',
        key: 'perfil',
  ...
  const filterColumns = useMemo<AppDataTableColumn<CrudListRecord, RestricoesProdutosFilters>[]>(() => [
    {
      id: 'q',
      ...
      filter: {
        ...
        key: 'q',
        label: t('common.search', 'Buscar'),
      } satisfies AppDataTableFilterConfig<RestricoesProdutosFilters>,
    } as AppDataTableColumn<CrudListRecord, RestricoesProdutosFilters>,
    ...columns,
  ], [columns, t])
    ...
    <div className="space-y-5">
      ...
      <AsyncState isLoading={listState.isLoading} error={listState.error}>
              ...
              <DataTablePageActions
                actions={[
                  ...
                  access.canCreate
                    ? {
                        label: t(
                          'maintenance.productRestrictions.actions.creationAssistant',
                          'Assistente de criação',
                        ),
                        icon: Sparkles,
                        href: '/restricoes-produtos/novo',
                        tone: 'primary',
                      }
                    : null,
                ]}
        ...
        >
          <DataTableFiltersCard
            variant="embedded"
            columns={filterColumns as AppDataTableColumn<unknown, RestricoesProdutosFilters>[]}
            draft={filtersDraft}
            applied={filters}
            expanded={filtersExpanded}
          ...
          <AppDataTable
            ...
            sort={{
              ...
              onToggle: (column) => tableState.toggleSort(column as RestricoesProdutosFilters['orderBy']),
            }}
                ...
                <div className="app-control rounded-[1rem] p-4">
                  ...
                  <AppDataTable
                    ...
                    columns={[
                      {
                        id: 'target',
                        label: t('maintenance.productRestrictions.sections.target', 'Alvo'),
                        cell: (rule) => describeSide(rule, TARGET_FIELDS),
                      },
                      {
                        id: 'products',
                        label: t('maintenance.productRestrictions.sections.products', 'Produtos'),
                        cell: (rule) => describeSide(rule, PRODUCT_FIELDS),
                      },
                      {
                        id: 'period',
                        label: t('maintenance.productRestrictions.sections.period', 'Per\u00edodo'),
                        cell: (rule) => (
                    ...
                    emptyMessage={t('maintenance.productRestrictions.emptyRules', 'Nenhuma regra encontrada.')}
                    mobileCard={{
            ...
            rowActions={(row) => [
              {
                ...
                icon: Pencil,
                href: `/restricoes-produtos/${String(row.id)}/editar`,
            ...
            mobileCard={{
              ...
              subtitle: (row) => String(row.perfil || 'todos'),
              meta: (row) => (
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          try {
            await restricoesProdutosClient.delete(ids)
            setConfirmDeleteIds(null)
            tableState.clearSelection()
            setFeedback({
              tone: 'success',
              message: t('maintenance.productRestrictions.deleteSuccess', 'Restrição removida com sucesso.'),
            })
            ...
            setFeedback({
              ...
              message: error instanceof Error
                ? error.message
                : t('maintenance.productRestrictions.deleteError', 'Não foi possível excluir a restrição.'),
            })
          }
        }}
      />
