...
function formatUpdateMeta(updatedAt: string, updatedBy: string, t: ReturnType<typeof useI18n>['t']) {
	...
	return t('integrationsAttendance.fields.lastUpdateValue', 'Última alteração: {{date}} por {{user}}').replace('{{date}}', updatedAt).replace('{{user}}', updatedBy);
}
					...
					<SectionCard className="px-4 py-4 md:px-5 md:py-5">
						<div className="overflow-x-auto">
							<div className="flex min-w-max items-center gap-2">
								...
								/>
								<TabButton
									active={activeTab === 'jivo'}
									icon={<MessageSquare className="h-4 w-4" />}
									label={t('integrationsAttendance.tabs.jivo', 'Jivo Chat')}
									onClick={() => setActiveTab('jivo')}
								/>
								<TabButton
									active={activeTab === 'ebit'}
									...
									<tbody>
										{branches.map((branch, index) => (
											<tr key={branch.id || `branch-${index}`} className="app-table-row-hover border-t border-line align-top">
												...
												<td className="px-4 py-3 text-xs text-slate-500">{formatUpdateMeta(branch.whatsappNumeroMeta.updatedAt, branch.whatsappNumeroMeta.updatedBy, t)}</td>
											</tr>
										))}
									</tbody>
								</table>
									...
									<div className="space-y-3">
										...
										{hasToken ? (
											<div className="flex flex-wrap gap-2">
												{!tokenEditable ? (
													<button type="button" className="app-button-secondary" onClick={() => setTokenEditable(true)} disabled={saving}>
						...
						>
							...
							>
								<>
									...
									/>
									<span className="text-xs text-slate-500">{formatUpdateMeta(initialRecord.metadata.jivoJs.updatedAt, initialRecord.metadata.jivoJs.updatedBy, t)}</span>
								</>
							</FormField>
						</SectionCard>
					) : null}
						...
						>
							...
							>
								<>
									...
									/>
									<span className="text-xs text-slate-500">{formatUpdateMeta(initialRecord.metadata.ebitCodigo.updatedAt, initialRecord.metadata.ebitCodigo.updatedBy, t)}</span>
								</>
							</FormField>
						</SectionCard>
					) : null}
