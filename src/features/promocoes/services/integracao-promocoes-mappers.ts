...
export const integracaoPromocoesParameterKeys = ['idever_client_id', 'idever_client_secret', 'idever_app_id', 'idever_app_secret', 'idever_rule_id'] as const;

export const integracaoPromocoesEncryptedKeys = ['idever_client_secret', 'idever_app_secret'] as const satisfies readonly IntegracaoPromocoesFieldKey[];
...
function createEmptyMetadata(): Record<IntegracaoPromocoesFieldKey, PromocoesFieldMeta> {
	return integracaoPromocoesParameterKeys.reduce(
		(accumulator, key) => {
...
export function getIntegracaoPromocoesEncryptedKeysToSave(initialValues: IntegracaoPromocoesValues, currentValues: IntegracaoPromocoesValues): IntegracaoPromocoesEncryptedKey[] {
	return integracaoPromocoesEncryptedKeys.filter((key) => currentValues[key] !== initialValues[key] || !initialValues[key].trim());
}
...
export function buildIntegracaoPromocoesSavePayload(
	...
	const payload: IntegracaoPromocoesParameterPayload[] = [{ id_filial: null, chave: 'versao', parametros: resolveTimestamp(), integracao: 0, criptografado: 0 }];
