...
export const integracaoLoginSocialParameterKeys = ['url_site', 'g_id_aplicacao', 'g_senha_aplicacao', 'fb_id_aplicacao', 'fb_senha_aplicacao'] as const;

export const integracaoLoginSocialWritableKeys = ['g_id_aplicacao', 'g_senha_aplicacao', 'fb_id_aplicacao', 'fb_senha_aplicacao'] as const;

export const integracaoLoginSocialEncryptedKeys = ['g_senha_aplicacao', 'fb_senha_aplicacao'] as const;
...
function createEmptyMetadata(): Record<IntegracaoLoginSocialFieldKey, LoginSocialFieldMeta> {
	return integracaoLoginSocialParameterKeys.reduce(
		(accumulator, key) => {
...
export function getIntegracaoLoginSocialEncryptedKeysToSave(
	initialValues: IntegracaoLoginSocialValues,
	currentValues: IntegracaoLoginSocialValues,
): IntegracaoLoginSocialEncryptedKey[] {
	return integracaoLoginSocialEncryptedKeys.filter((key) => currentValues[key] !== initialValues[key] || !initialValues[key].trim());
}
...
export function buildIntegracaoLoginSocialSavePayload(
	...
	const payload: IntegracaoLoginSocialParameterPayload[] = [{ id_filial: null, chave: 'versao', parametros: resolveTimestamp(), integracao: 0, criptografado: 0 }];
