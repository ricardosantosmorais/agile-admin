...
export const integracaoComErpParametrosClient = {
	async get() {
		const payload = await httpClient<unknown>('/api/integracao-com-erp/parametros', {
			method: 'GET',
			cache: 'no-store',
		});
	...
	async save(fields: IntegracaoComErpConfigFieldDefinition[], initialValues: Record<string, string>, currentValues: Record<string, string>) {
		...
		return httpClient('/api/integracao-com-erp/parametros', {
			method: 'POST',
