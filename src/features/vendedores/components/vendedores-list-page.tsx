				...
				<SectionCard
					action={
						<div className="flex w-full items-center justify-between gap-3">
							...
							/>
							<DataTablePageActions actions={[access.canCreate ? { label: t('common.new', 'Novo'), icon: Plus, href: '/vendedores/novo', tone: 'primary' } : null]} />
						</div>
					}
				>
					...
					<AppDataTable<CrudListRecord, string, CrudListFilters>
						...
						rowActions={(record) => [
							{
								id: 'edit',
								label: access.canEdit ? t('simpleCrud.actions.edit', 'Editar') : t('simpleCrud.actions.view', 'Visualizar'),
								icon: access.canEdit ? Pencil : SearchIcon,
								href: `/vendedores/${record.id}/editar`,
								visible: access.canEdit || access.canView,
							},
							{
								id: 'users',
								label: t('people.sellers.actions.linkedUsers', 'Usuários vinculados'),
								icon: Users,
								onClick: () => void openLinkedUsers(record),
								visible: access.canEdit || access.canView,
							},
							{
								id: 'delete',
						...
						renderExpandedRow={
							VENDEDORES_CONFIG.details?.length
								? (record) => (
