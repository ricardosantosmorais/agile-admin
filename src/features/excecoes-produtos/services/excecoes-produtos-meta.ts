...
export const EXCECAO_PRODUTO_AUDIENCE_TYPES: Array<{
  value: ExcecaoProdutoAudienceType
  label: string
...
export const EXCECAO_PRODUTO_PRODUCT_TYPES: Array<{
  value: ExcecaoProdutoProductType
  label: string
  resource?: CrudResource
}> = [
  { value: 'todos', label: 'Todos' },
...
export function getAudienceMeta(type: ExcecaoProdutoAudienceType) {
  return EXCECAO_PRODUTO_AUDIENCE_TYPES.find((item) => item.value === type) ?? EXCECAO_PRODUTO_AUDIENCE_TYPES[0]
}

export function getProductMeta(type: ExcecaoProdutoProductType) {
  return EXCECAO_PRODUTO_PRODUCT_TYPES.find((item) => item.value === type) ?? EXCECAO_PRODUTO_PRODUCT_TYPES[0]
}
