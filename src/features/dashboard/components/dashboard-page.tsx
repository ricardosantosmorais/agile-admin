          ...
          <tr>
            {columns.map((column) => (
              <th
                key={column.key}
                className={[
                  'px-3.5 py-2.5 font-semibold uppercase tracking-[0.18em]',
              ...
              {columns.map((column) => (
                <td
                  ...
                  className={[
                    'px-3.5 py-2.5',
                    column.align === 'right' ? 'text-right' : 'text-left',
                    column.key === columns[0]?.key ? 'font-semibold text-slate-950' : '',
                  ].join(' ')}
                >
                  {row[column.key]}
                </td>
...
export function DashboardPage() {
  ...
  const hasSeriesData = hasMeaningfulRows(seriesRows, ['atual', 'anterior'])
  ...
  return (
    <div ref={dashboardRef} className={['space-y-4 xl:space-y-5', isExportingPdf ? 'dashboard-exporting' : ''].join(' ')}>
      <PageHeader
        actions={
          <>
            <DateRangePicker value={selectedRange} onChange={setSelectedRange} presets={presetRanges} maxDays={90} />
            <button
              type="button"
          ...
          <>
            ...
            >
              <div className="space-y-4 xl:space-y-5">
                <SectionCard
                  ...
                  description={t('dashboard.dailyRevenueDescription', 'Comparando o periodo selecionado ({{range}}) com o periodo anterior equivalente.', { range: snapshot.rangeLabel })}
                >
                    ...
                    {hasSeriesData ? (
                      <ResponsiveChart>
                        <AreaChart data={seriesRows}>
                          ...
                          <Tooltip
                            content={
                              <CustomTooltip
                                lines={(row) => [
                                  row.data
                                    ? `${formatDateLabel(row.data)} (${formatWeekday(row.data, locale)})`
                                    : row.dia || '',
                          ...
                          <Area type="monotone" dataKey="anterior" name={t('dashboard.previousPeriod', 'Previous period')} stroke="#94a3b8" fill="#e2e8f0" strokeDasharray="6 6" />
                        </AreaChart>
                      </ResponsiveChart>
                    ) : (
                      <ChartEmptyState
            ...
            >
              <div className="grid gap-4 xl:grid-cols-2">
                              ...
                              <CustomTooltip
                                lines={(row) => [
                                  ...
                                  row.name === 'Carrinho' || row.name === 'Aprovados'
                                    ? ''
                                    : `Aproveitamento sobre aprovados: ${formatPercent(row.pctAprovados ?? 0)}`,
                                ]}
                              />
                            }
                          />
                ...
                <SectionCard title={t('dashboard.ticketPerDayTitle', 'Ticket medio por dia')} description={t('dashboard.ticketPerDayDescription', 'Evolucao do valor medio por pedido ao longo do periodo.')}>
                  <ChartFrame>
                    {hasTicketData ? (
                      <ResponsiveChart>
            ...
            >
              <div className="grid gap-4 xl:grid-cols-2">
                ...
                <SectionCard title={t('dashboard.issuerSalesTitle', 'Vendas por emitente')} description={t('dashboard.issuerSalesDescription', 'Compara pedidos feitos pelo cliente com pedidos feitos com apoio de vendedor.')}>
                  <ChartFrame>
                    {hasEmitenteData ? (
                      <ResponsiveChart>
              ...
              <SectionCard title={t('dashboard.marketingOverviewTitle', 'Visao de marketing')} description={t('dashboard.marketingOverviewDescription', 'Indicadores de incentivo e impacto comercial do periodo.')}>
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  {snapshot.marketingMetrics.map((metric) => (
                    <StatCard key={metric.label} {...metric} description={undefined} />
                  ))}
                </div>
              </SectionCard>
            </LazyDashboardSection>
            ...
            >
              <div className="space-y-4 xl:space-y-5">
                ...
                <div className="grid gap-4 xl:grid-cols-2">
                  <SectionCard title={t('dashboard.incentiveRevenueTitle', 'Receita por tipo de incentivo')} description={t('dashboard.incentiveRevenueDescription', 'Receita dos itens com cada tipo de incentivo.')}>
                    <ChartFrame>
                  ...
                  <SectionCard title={t('dashboard.ticketComparisonTitle', 'Ticket medio')} description={t('dashboard.ticketComparisonDescription', 'Comparativo entre pedidos com e sem incentivo.')}>
                    <ChartFrame>
                            ...
                            <Tooltip
                              content={
                                <CustomTooltip
                                  lines={(row) => [
                                    row.name || '',
                                    `${t('dashboard.tooltip.averageTicket', 'Average ticket')}: ${formatCurrency(row.value ?? 0)}`,
                                  ]}
                                />
                              }
                            />
            ...
            >
              <div className="grid gap-4 xl:grid-cols-2">
                            ...
                            content={
                              <CustomTooltip
                                lines={(row) => [
                                  ...
                                  row.fonte
                                    ? `${t('dashboard.tooltip.source', 'Source')}: ${row.fonte === 'automatico'
                                      ? t('dashboard.couponAutomatic', 'Automatic coupon')
                                      : t('dashboard.couponManual', 'Manual coupon')}`
                                    : '',
                ...
                <SectionCard title={t('dashboard.topPromotionsTitle', 'Top promocoes por receita')} description={t('dashboard.topPromotionsDescription', 'Promocoes com maior impacto em receita no periodo.')}>
                  <ChartFrame>
                    {hasPromotionData ? (
                      <ResponsiveChart>
