...
function getModelUri(monaco: Monaco, editorId: string, language: 'html' | 'css') {
  ...
  const uri = monaco.Uri.parse(`inmemory://layout-editor/${editorId}.${language}`)
  modelUriCache.set(cacheKey, uri.toString())
  return uri
}

function getOrCreateModel(monaco: Monaco, editorId: string, language: 'html' | 'css', value: string) {
  ...
  return (
    ...
    >
      <Editor
        ...
        options={options}
        path={`layout-editor-${editorId}.${language}`}
      />
    </div>
  )
}
