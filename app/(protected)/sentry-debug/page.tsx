	...
	return (
		<main className="mx-auto flex min-h-full w-full max-w-3xl flex-col gap-6 px-6 py-10">
			<header className="space-y-2">
				...
				<p className="text-sm text-slate-600">Use esta tela para validar manualmente a integração com o Sentry no client e no server.</p>
			</header>
			...
			{disabled ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">Esta rota fica disponível apenas fora de produção.</div> : null}
			...
			<section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
				...
				{clientState.message ? <p className={`mt-4 text-sm ${clientState.kind === 'error' ? 'text-rose-600' : 'text-emerald-700'}`}>{clientState.message}</p> : null}
			</section>
			...
			<section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
				...
				{serverState.message ? <p className={`mt-4 text-sm ${serverState.kind === 'error' ? 'text-rose-600' : 'text-emerald-700'}`}>{serverState.message}</p> : null}
			</section>
		</main>
	);
}
