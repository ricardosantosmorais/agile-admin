...
import { POST } from '@/app/api/excecoes-produtos/wizard/route'
...
describe('excecoes-produtos wizard route', () => {
  beforeEach(() => {
  ...
  it('normalizes active weekday HH:MM values to API time format and clears inactive times', async () => {
    const request = new NextRequest('http://localhost/api/excecoes-produtos/wizard', {
      method: 'POST',
      body: JSON.stringify({
        id: 'parent-1',
        rows: [{
          motivo: 'Excecao por horario',
          seg: 1,
