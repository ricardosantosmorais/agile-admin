...
export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
	...
	const result = await agileV2Fetch('parametros_cadastro', {
		method: 'GET',
		query: new URLSearchParams({ perpage: '1', id }),
	})

	if (!result.ok) {
		return NextResponse.json({ message: getPayloadMessage(result.payload, 'Não foi possível carregar o parâmetro.') }, { status: result.status || 400 })
	}
	...
	if (!item) {
		return NextResponse.json({ message: 'Parâmetro não encontrado.' }, { status: 404 })
	}
