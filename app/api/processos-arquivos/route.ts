...
const MAX_SPREADSHEET_SIZE_BYTES = 500 * 1024 * 1024
const UPLOAD_STATE_DIR = path.join(os.tmpdir(), 'admin-v2-web-processos-arquivos-upload')
...
export async function GET(request: NextRequest) {
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'Não foi possível carregar os processos de importação.') },
...
export async function POST(request: NextRequest) {
  ...
  if (!(file instanceof File)) {
    return NextResponse.json({ message: 'Arquivo da planilha não informado.' }, { status: 400 })
  }

  if (!isSpreadsheetFile(file)) {
    return NextResponse.json({ message: 'Envie somente arquivo no formato XLS ou XLSX.' }, { status: 400 })
  }
  ...
  if (!isMultipartUpload) {
    ...
    if (!createProcess.ok) {
      return NextResponse.json(
        { message: getErrorMessage(createProcess.payload, 'Não foi possível criar o processo de importação.') },
