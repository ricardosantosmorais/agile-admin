...
export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
	...
	if (!result.ok) {
		return NextResponse.json({ message: getPayloadMessage(result.payload, 'Não foi possível carregar o script.') }, { status: result.status || 400 })
	}

	const item = asArray<Record<string, unknown>>(asRecord(result.payload).data).at(0)
	if (!item) {
		return NextResponse.json({ message: 'Script não encontrado.' }, { status: 404 })
	}
