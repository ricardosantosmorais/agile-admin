...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const body = await request.json() as { items?: Array<{ id_produto?: string; quantidade?: number | null; posicao?: number | null }> }
  const items = Array.isArray(body.items) ? body.items : []

  const result = await serverApiFetch('listas/produtos', {
    method: 'POST',
    token: session.token,
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!session) return NextResponse.json({ message: 'Sessao expirada.' }, { status: 401 })
  const { id } = await context.params
  const body = await request.json() as { items?: Array<{ id_produto?: string }> }
  const items = Array.isArray(body.items) ? body.items : []
