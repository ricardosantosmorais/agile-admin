...
const INTEGRACAO_ATENDIMENTO_PARAMETER_KEYS = [
  'whatsapp_numero',
  'whatsapp_id_numero',
  'whatsapp_exibicao',
  'whatsapp_gateway',
...
export async function GET() {
  ...
  const [parametersResult, branchesResult] = await Promise.all([
    serverApiFetch(buildCompanyParametersPath(session.currentTenantId, [...INTEGRACAO_ATENDIMENTO_PARAMETER_KEYS]), {
      method: 'GET',
      token: session.token,
      tenantId: session.currentTenantId,
    }),
    serverApiFetch(buildLookupPath('filiais', session.currentTenantId, {
      order: 'nome_fantasia',
  ...
  if (!parametersResult.ok) {
    return NextResponse.json(
      { message: getErrorMessage(parametersResult.payload, 'Não foi possível carregar os parâmetros de atendimento.') },
  ...
  if (!branchesResult.ok) {
    return NextResponse.json(
      { message: getErrorMessage(branchesResult.payload, 'Não foi possível carregar as filiais para atendimento.') },
      { status: branchesResult.status || 400 },
    )
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'Não foi possível salvar as configurações de atendimento.') },
