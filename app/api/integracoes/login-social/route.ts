...
import { integracaoLoginSocialParameterKeys } from '@/src/features/login-social/services/integracao-login-social-mappers';
...
export async function GET() {
	...
	const result = await serverApiFetch(buildCompanyParametersPath(session.currentTenantId, [...integracaoLoginSocialParameterKeys]), {
		method: 'GET',
		token: session.token,
	...
	if (!result.ok) {
		return NextResponse.json({ message: getErrorMessage(result.payload, 'Não foi possível carregar os parâmetros de login social.') }, { status: result.status || 400 });
	}
...
export async function POST(request: NextRequest) {
	...
	if (!result.ok) {
		return NextResponse.json({ message: getErrorMessage(result.payload, 'Não foi possível salvar as configurações de login social.') }, { status: result.status || 400 });
	}
