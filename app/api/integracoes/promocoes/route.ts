...
import { integracaoPromocoesParameterKeys } from '@/src/features/promocoes/services/integracao-promocoes-mappers';
...
export async function GET() {
	...
	const result = await serverApiFetch(buildCompanyParametersPath(session.currentTenantId, [...integracaoPromocoesParameterKeys]), {
		method: 'GET',
		token: session.token,
	...
	if (!result.ok) {
		return NextResponse.json({ message: getErrorMessage(result.payload, 'Não foi possível carregar os parâmetros de promoções.') }, { status: result.status || 400 });
	}
...
export async function POST(request: NextRequest) {
	...
	if (!result.ok) {
		return NextResponse.json({ message: getErrorMessage(result.payload, 'Não foi possível salvar as configurações de promoções.') }, { status: result.status || 400 });
	}
