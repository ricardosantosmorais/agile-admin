...
import { integracaoMarketingParameterKeys } from '@/src/features/marketing/services/integracao-marketing-mappers';
...
export async function GET() {
	...
	const result = await serverApiFetch(buildCompanyParametersPath(session.currentTenantId, [...integracaoMarketingParameterKeys]), {
		method: 'GET',
		token: session.token,
	...
	if (!result.ok) {
		return NextResponse.json({ message: getErrorMessage(result.payload, 'Não foi possível carregar os parâmetros de marketing.') }, { status: result.status || 400 });
	}
...
export async function POST(request: NextRequest) {
	...
	if (!result.ok) {
		return NextResponse.json({ message: getErrorMessage(result.payload, 'Não foi possível salvar as configurações de marketing.') }, { status: result.status || 400 });
	}
