...
export async function GET(request: NextRequest) {
  ...
  const result = await serverApiFetch(`promocoes/excecoes?id_promocao=${encodeURIComponent(promotionId)}&page=1&perpage=200&embed=filial,praca,condicao_pagamento,cliente,forma_pagamento,grupo,rede,segmento,supervisor,tabela_preco,vendedor`, {
    method: 'GET',
    token: session.token,
...
export async function DELETE(request: NextRequest) {
  ...
  const result = await serverApiFetch('promocoes/excecoes', {
    ...
    token: session.token,
    tenantId: session.currentTenantId,
    body: ids.map((itemId) => ({ id_promocao: promotionId, id: itemId })),
  })
