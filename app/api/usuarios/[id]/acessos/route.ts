...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const params = new URLSearchParams({
    ...
    order: searchParams.get('orderBy') || 'ultimo_acesso',
  ...
  const result = await serverApiFetch(`usuarios/acessos?${params.toString()}`, {
    method: 'GET',
    token: session.token,
  ...
  if (!result.ok) {
    return NextResponse.json({ message: getErrorMessage(result.payload, 'Nao foi possivel carregar os acessos do usuario.') }, { status: result.status || 400 })
  }
