...
import type { RestricaoProdutoApiRow } from '@/src/features/restricoes-produtos/services/restricoes-produtos-types'
...
function sanitizeRow(row: RestricaoProdutoApiRow, parentId: string, tenantId: string, index: number) {
  const next: Record<string, unknown> = {
    ...
    id_empresa: tenantId,
    perfil: typeof row.perfil === 'string' && row.perfil.trim() ? row.perfil.trim() : 'todos',
  ...
  if (next.pedido_minimo === '' || next.pedido_minimo === undefined) {
    next.pedido_minimo = null
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json() as { id?: string; rows?: RestricaoProdutoApiRow[]; deleteIds?: string[] }
  ...
  if (deleteIds.length) {
    const deleteResult = await serverApiFetch('restricoes_produtos', {
      method: 'DELETE',
      token: session.token,
  ...
  if (!result.ok) {
    return NextResponse.json({ message: mapError(result.payload, 'Não foi possível salvar a restrição.') }, { status: result.status || 400 })
  }
