...
import { configuracoesPedidosParameterKeys } from '@/src/features/configuracoes-pedidos/services/configuracoes-pedidos-mappers'
...
export async function GET() {
  ...
  const result = await serverApiFetch(buildCompanyParametersPath(session.currentTenantId, configuracoesPedidosParameterKeys), {
    method: 'GET',
    token: session.token,
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel carregar as configuraÃ§Ãµes de pedidos.') },
...
export async function POST(request: NextRequest) {
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel salvar as configuraÃ§Ãµes de pedidos.') },
      { status: result.status || 400 },
    )
  }
