...
import { configuracoesAssistenteVirtualFieldKeys } from '@/src/features/configuracoes-assistente-virtual/services/configuracoes-assistente-virtual-mappers'
...
export async function GET() {
  ...
  const result = await serverApiFetch(buildCompanyParametersPath(session.currentTenantId, configuracoesAssistenteVirtualFieldKeys), {
    method: 'GET',
    token: session.token,
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel carregar as configuraÃ§Ãµes do assistente virtual.') },
...
export async function POST(request: NextRequest) {
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel salvar as configuraÃ§Ãµes do assistente virtual.') },
      { status: result.status || 400 },
    )
  }
