...
import { configuracoesProdutosParameterKeys } from '@/src/features/configuracoes-produtos/services/configuracoes-produtos-mappers'
...
export async function GET() {
  ...
  const result = await serverApiFetch(buildCompanyParametersPath(session.currentTenantId, configuracoesProdutosParameterKeys), {
    method: 'GET',
    token: session.token,
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel carregar as configuraÃ§Ãµes de produtos.') },
...
export async function POST(request: NextRequest) {
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel salvar as configuraÃ§Ãµes de produtos.') },
      { status: result.status || 400 },
    )
  }
