  ...
  if (!companyResult.ok) {
    return {
      error: NextResponse.json(
        { message: getErrorMessage(companyResult.payload, 'NÃƒÂ£o foi possÃƒÂ­vel carregar os dados da empresa.') },
...
export async function GET() {
  ...
  if (!schemaResult.ok) {
    return NextResponse.json(
      { message: getErrorMessage(schemaResult.payload, 'NÃƒÂ£o foi possÃƒÂ­vel carregar o schema de configuraÃƒÂ§ÃƒÂµes gerais.') },
  ...
  if (!parametersResult.ok) {
    return NextResponse.json(
      { message: getErrorMessage(parametersResult.payload, 'NÃƒÂ£o foi possÃƒÂ­vel carregar os parÃƒÂ¢metros gerais.') },
...
export async function POST(request: NextRequest) {
  ...
  if (companyId) {
    ...
    if (!companyResult.ok) {
      return NextResponse.json(
        { message: getErrorMessage(companyResult.payload, 'NÃƒÂ£o foi possÃƒÂ­vel salvar os dados da empresa.') },
        { status: companyResult.status || 400 },
      )
    }
  }
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃƒÂ£o foi possÃƒÂ­vel salvar os parÃƒÂ¢metros gerais.') },
