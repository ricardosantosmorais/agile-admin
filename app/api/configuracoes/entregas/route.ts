...
import { buildApiInQuery, buildLookupPath, buildCompanyParametersPath, extractParameterValues } from '@/src/lib/company-parameters-query';
...
export async function GET() {
	...
	if (!parametersResult.ok) {
		return NextResponse.json(
			{ message: getErrorMessage(parametersResult.payload, 'Não foi possível carregar as configurações de entregas.') },
	...
	const deliveryMethodsResult = !deliveryMethodIds.length
		...
		: await serverApiFetch(
				buildLookupPath('formas_entrega', session.currentTenantId, {
	...
	if (!deliveryMethodsResult.ok) {
		return NextResponse.json(
			{ message: getErrorMessage(deliveryMethodsResult.payload, 'Não foi possível carregar as formas de entrega.') },
...
export async function POST(request: NextRequest) {
	...
	if (!result.ok) {
		return NextResponse.json({ message: getErrorMessage(result.payload, 'Não foi possível salvar as configurações de entregas.') }, { status: result.status || 400 });
	}
