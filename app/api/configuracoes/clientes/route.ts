...
import { configuracoesClientesParameterKeys } from '@/src/features/configuracoes-clientes/services/configuracoes-clientes-mappers'
...
export async function GET() {
  ...
  const result = await serverApiFetch(buildCompanyParametersPath(session.currentTenantId, configuracoesClientesParameterKeys), {
    method: 'GET',
    token: session.token,
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel carregar as configuraÃ§Ãµes de clientes.') },
...
export async function POST(request: NextRequest) {
  ...
  if (!result.ok) {
    return NextResponse.json(
      { message: getErrorMessage(result.payload, 'NÃ£o foi possÃ­vel salvar as configuraÃ§Ãµes de clientes.') },
      { status: result.status || 400 },
    )
  }
