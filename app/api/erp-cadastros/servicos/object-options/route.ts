...
import { loadServicoObjectOptions } from '@/app/api/erp-cadastros/servicos/_servicos-shared'

export async function GET(request: NextRequest) {
	const sessionOrResponse = await requireRootAgileSession()
	if (sessionOrResponse instanceof NextResponse) return sessionOrResponse
	const params = request.nextUrl.searchParams
	const options = await loadServicoObjectOptions(
		(params.get('tipo_objeto') || '').trim(),
