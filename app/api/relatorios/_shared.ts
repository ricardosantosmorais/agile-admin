...
import { decryptLegacyAdminValue } from '@/src/lib/legacy-crypto'
...
export async function resolveRelatorioContext(id: string) {
  try {
    ...
    const parametrosResponse = await serverApiFetch(
      `empresas/parametros?id_empresa=${encodeURIComponent(session.currentTenantId)}&order=chave,posicao&perpage=1000`,
      {
        method: 'GET',
        token: session.token,
        tenantId: session.currentTenantId,
    ...
    if (!encryptedPortalToken) {
      return {
        error: NextResponse.json(
          { message: 'Parâmetro portal_token não configurado para a empresa ativa.' },
          { status: 409 },
        ),
      }
    }
    ...
    try {
      ...
      return {
        error: NextResponse.json(
          { message: error instanceof Error ? error.message : 'Não foi possível descriptografar o portal_token.' },
          { status: 500 },
        ),
      }
    }
    ...
    if (!portalToken) {
      return {
        error: NextResponse.json(
          { message: 'portal_token inválido para a empresa ativa.' },
          { status: 409 },
        ),
      }
    }
    ...
    if (!reportApi) {
      return {
        error: NextResponse.json(
          { message: 'O relatório não possui integração externa configurada.' },
          { status: 409 },
        ),
      }
    }
    ...
    return {
      error: NextResponse.json(
        { message: error instanceof Error ? error.message : 'Não foi possível carregar o relatório.' },
        { status: 500 },
      ),
    }
  }
...
export function buildDynamicProcessFields(
  fields: RelatorioFiltroDinamico[],
  ...
  for (const field of fields) {
    ...
    if (field.tipo === 'valor') {
      const normalizeMoney = (input: string) => input.replace(/\./g, '').replace(',', '.').replace(/[^\d.-]/g, '')
      const start = String(valores[`${field.campo}__start`] || '').trim()
      const end = String(valores[`${field.campo}__end`] || '').trim()
